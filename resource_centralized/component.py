...
'''
Contains class for a simulation platform component used to simulate simple load and generator whose published states are determined by a file containing a simple time series of attribute values for each epoch.
'''
import asyncio
...
from resource_centralized.components import SimulationComponents
...
class StaticTimeSeriesResourceCentralized(AbstractSimulationComponent):
    '''
    A simulation platform component used to simulate simple load and generator whose published states are determined by a file containing a simple time series of attribute values for each epoch.
    '''
    ...
    def __init__(self, resource_state_component_ids: str, resource_state_csv_folder: str,
        ...
        '''
        Create components which use the given csv source for its published resource states.
        '''
        super().__init__()
        ...
        # publish resource states to these topics
        self._result_topics = {}

        for component in self._resource_state_component_ids:
        ...
        for component, resource_state_type in zip(self._resource_state_component_ids, self._types):
            self._result_topics[component] = '.'.join( [ self._resource_state_topic, resource_state_type, component ])
    ...
    async def _send_resource_state_message(self):
        ...
        '''
        for component in self._resource_state_component_ids:
            resourceState = self._get_resource_state_message(component)
            await self._rabbitmq_client.send_message(self._result_topics[component], resourceState.bytes())
        
    def _get_resource_state_message(self, component) -> ResourceStateMessage:
        '''
        Create a ResourceStateMessage from the next resource state info available from the state source.
        '''
        state = self.__simulation_components.get_component(component).getNextEpochData()
...
def create_component() -> StaticTimeSeriesResourceCentralized:
    '''
    Create a StaticTimeSeriesResourceCentralized. Initialized with a folder that contains csv files for components.
    '''
    # get information about the used source csv file, component names and types. 
    ...
    return StaticTimeSeriesResourceCentralized(
...
async def start_component():
    '''
    Start a StaticTimeSeriesResourceCentralized component.
    '''
